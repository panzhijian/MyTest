...
public class BubbleChartRenderer: ChartDataRendererBase
{
                  ...
                  normalizeSize: Bool) -> CGFloat
    {
        let factor: CGFloat = normalizeSize
            ...
            : entrySize
        var shapeSize: CGFloat = reference * factor
        
        shapeSize = 15;//cy 限制大小为15
        return shapeSize
    }
...
//                let maxx = min(dataSet.entryIndex(entry: entryTo) + 1, entryCount)
                ...
                for j in minx ..< maxx
                {
                    ...
                    var text = formatter.stringFromNumber(e.size)
                    let array = text!.componentsSeparatedByCharactersInSet(NSCharacterSet (charactersInString:"."))
                    text = array[0]
