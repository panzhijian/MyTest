...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (WVJBMessage* message in messages) {
        ...
        if (responseId) {
            ...
            if (!handler) {
                NSLog(@"调用了WebViewJavascriptBridge内部别人写的方法WVJBNoHandlerException, No handler for message from JS: %@", message);
                
                if(message[@"data"][@"umengUrl"])
                {
                    
                [self setUmengWithUrl:message[@"data"][@"umengUrl"] shareText:message[@"data"][@"context"]];
                }
                else if(message[@"data"][@"openUrl"])
                {
                    [[UIApplication sharedApplication] openURL:message[@"data"][@"openUrl"]];
                }
                else if(message[@"data"][@"alertPhoto"])
                {
                    [[UIApplication sharedApplication] openURL:message[@"data"][@"alertPhoto"]];
                }
...
- (void)setUmengWithUrl:(NSString *)url shareText:(NSString *)shareText
{
    ...
    [UMSocialSnsService presentSnsIconSheetView:[[[UIApplication sharedApplication] keyWindow] rootViewController]
                                         appKey:UMENG_KEY
                                      shareText:[NSString stringWithFormat:@"%@[%@]",shareText,url]
